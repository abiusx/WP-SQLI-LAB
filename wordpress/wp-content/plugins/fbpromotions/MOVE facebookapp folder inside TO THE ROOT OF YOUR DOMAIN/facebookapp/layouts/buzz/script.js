...
var scriptRows = null;
var scriptCols = null;
var scriptGalleryarray = null;
var scriptPageCount = null;
...
function photogallery(garray, cols, rows, twidth, theight, paginatetext){
	...
	scriptPageCount = this.pagecount;
...
photogallery.prototype.showpage=function(gdiv, pagenumber){
	currentPageTop = pagenumber;
	
	var totalitems= scriptGalleryarray.length //total number of images
	var showstartindex=pagenumber*(scriptRows * scriptCols) //array index of div to start showing per pagenumber setting
	var showendindex=showstartindex+(scriptRows * scriptCols) //array index of div to stop showing after per pagenumber setting
	...
	for (var i=showstartindex, currentcell=0; i<showendindex && i<totalitems; i++, currentcell++) //Loop thru this page's images and populate cells with them
		tablecells[currentcell].innerHTML= photogallery.prototype.createImage( scriptGalleryarray[i] )
...
function next(){
	var gdiv=document.getElementById("photogallery-"+ scriptGCount );
	var pdiv=document.getElementById("photogallerypaginate-"+this.gcount)
	//alert( gdiv.innerHTML + "  pageNumber:" + currentPageTop );	
	if( scriptPageCount > currentPageTop + 1 )
	{
		photogallery.prototype.showpage(  gdiv, currentPageTop+1 );
		var navlinks=pdiv.getElementsByTagName("a")
		navlinks[currentPageTop-1].className="";
		navlinks[currentPageTop].className="current";		
	}
}
