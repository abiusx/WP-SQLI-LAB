<?php 
	...
	<input name="post_java_id" id="post_java_id" type="hidden"  />
		...
		*/
		...
		$init_letter = $_GET['R'];
		$sql = "select * from ".$table_prefix."terms wpt,".$table_prefix."term_taxonomy wptt where wpt.name like '".$init_letter."%'   and wptt.taxonomy = 'category' and wpt.term_id = wptt.term_id and wpt.term_id not in(".$lists.")";
		}
		else
		{
			$csql_rec = $wpdb->get_results("select * from ".$table_prefix."terms where name='Conditions'");
			...
			foreach($sql_rec as $c_rec){
                ...
                <div class="categorylink">
					...
					<?php
						...
						#echo $psql;
						$psql_rec = $wpdb->get_results($psql);
						$i=1;
						$pcontent='';
