...
if(($_GET['do'] == "editgroup") && (!isset($_POST['edit_save_group']))){
	...
	$group_name = stripslashes($mingleforum->get_groupname($_GET['groupid']));
	global $wpdb, $table_prefix;
	...
	echo "<table class='form-table'>
			...
			<tr>
				...
				<td><textarea name='edit_group_description' ".ADMIN_ROW_COL.">".stripslashes($mingleforum->get_group_description($_GET['groupid']))."</textarea></td>
			</tr>
			<tr>
...
if(($_GET['do'] == "editforum") && (!isset($_POST['edit_save_forum']))){
	...
	echo "<h2>".__("Edit forum", "mingleforum")." \"".stripslashes($mingleforum->get_forumname($_GET['forumid']))."\"</h2>";
	echo "<form id='edit_forum_form' name='edit_forum_form' action='' method='post'>";
	
	echo "<table class='form-table'>";
	echo "<tr>
			<th>".__("Name:", "mingleforum")."</th>
			<td><input type='text' name='edit_forum_name' value='".stripslashes($mingleforum->get_forumname($_GET['forumid']))."' /></td>
		</tr>
		<tr>
			<th>".__("Description:", "mingleforum")."</th>
			<td><textarea name='edit_forum_description' ".ADMIN_ROW_COL.">".stripslashes($mingleforum->get_forum_description($_GET['forumid']))."</textarea></td>
		</tr>
		<tr>
			<th></th>
