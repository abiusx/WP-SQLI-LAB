...
class mingleforum{
	...
	function init(){
		...
		$this->profile_link 	= get_option('siteurl')."/wp-admin/profile.php";
	...
	global $wp_rewrite;
		...
		{
			$this->reg_link 		= get_option('siteurl')."/wp-login.php?action=register";
		}
		//END MINGLE REG LINK
		$this->topic_feed_url	= WPFURL."feed.php?topic=";
		$this->global_feed_url	= WPFURL."feed.php?topic=all";
		$this->home_url 		= $perm;
		$this->logout_link 		= get_option('siteurl')."/wp-login.php?action=logout&redirect_to=".get_permalink($this->get_pageid());
	}
	...
	function go() {
		...
		if(isset($_GET['mingleforumaction']))
			...
			$action = false;
		if (!isset($_GET['getNewForumID']) && !isset($_GET['delete_topic']) && !isset($_GET['remove_post']))
		{
			if ($action != false)
			{
				if ($this->options['forum_use_seo_friendly_urls'])
		...
		if($posts = $this->get_posts($thread_id)){
			if($user_ID){
				...
				if($this->array_search($this->current_thread, (array)$op["notify_topics"], true))
					...
					$this->notify_msg = __("Add this topic to your email notifications?", "mingleforum");
			}
	...
	function input_filter($string){
		global $wpdb;
		$Find = array("<", "%", "$", "onerror=");
		$Replace = array("&#60;", "&#37;", "&#36;", "onerror&#61;");
		$newStr = str_replace($Find, $Replace, $string);
		return $wpdb->escape($newStr);
	}
	...
	function topic_menu($thread, $pos = "top"){
		...
		if($user_ID || $this->allow_unreg()){	
			...
			if($this->options['forum_use_seo_friendly_urls'])
			{
				...
				{
					$op = get_user_meta($user_ID, "wpf_useroptions", true);
					if($this->array_search($this->current_thread, (array)$op["notify_topics"], true)) //Check if user has already subscribed to topic
						$menu .= "<td class='".$class."_back' nowrap='nowrap'><a onclick='return notify();' href='".$this->thread_link.$this->current_thread.".".$this->curr_page."&notify&id=$this->current_thread'>".__("Unsubscribe", "mingleforum")."</a></td>";
					else
						$menu .= "<td class='".$class."_back' nowrap='nowrap'><a onclick='return notify();' href='".$this->thread_link.$this->current_thread.".".$this->curr_page."&notify&id=$this->current_thread'>".__("Subscribe", "mingleforum")."</a></td>";
				}
				if($this->options['forum_use_rss'])
	...
	function setup_menu(){
		...
		if($this->options['forum_use_seo_friendly_urls'])
		{
			$menuitems = array(
								...
								"profile" 	=> $link,
								"search" 	=> "<a id='search_button' href='$this->base_url"."search'>".__("Search", "mingleforum")."</a>",
...
/****************************************************/
	...
	global $wpdb;
		...
		if($this->current_view == SEARCH){
			$terms = "";
			if(isset($_POST['wpf_search_string']))
				$terms = $wpdb->escape($_POST['wpf_search_string']);
...
DISABLED THIS IN 1.0.25*/
		...
		$groups = $this->get_groups();
			$o .= "<div class='wpf' style='margin:0 auto;'><form name='wpf_searchform' method='post' action=''>
					<table class='wpf-table search' cellspacing='0' cellpadding='0' width='100%'>
						<tr>
							<th colspan='2' class='wpf-bright'>".__("Search", "mingleforum")."</th>
						</tr>
						<tr>
							<td>
								<strong>".__("Search for:", "mingleforum")."</strong><br />
								<input type='text' name='search_words' size='30' class='wpf-input'/>
							</td>
							<td>
								<strong>".__("By user:", "mingleforum")."</strong><br />
								<input type='text' name='search_user' value='*' size='30' class='wpf-input'/>
							</td>
						</tr>
							<td colspan='2'>
								<strong>".__("Message Age:", "mingleforum")."</strong><br />
								".__("Between", "mingleforum")." <input type='text' size='5' value='0' name='search_min' class='wpf-input'/> ".__("and", "mingleforum"). "<input type='text' size='5' value='9999' name='search_max' class='wpf-input'/> ".__("days", "mingleforum")."
							</td>
						<tr>
						<tr>
							<td colspan='2'>
								<div style='padding:10px;' >
									 <a id='search_coll' href='javascript:void(0);' onclick='expandCollapseBoards(); return false;'><b> ".__("Click to choose Forum(s) to  search", "mingleforum")."</b></a><br />";
							...
							$o .= "</table>
									<input type='checkbox' id='check_all' name='check_all' checked='checked' onclick='invertAll(this, this.form, \"forum\");' /> ".__("Check all", "mingleforum")."
								</div>
							</td>
						</tr>
						</tr>
							<td colspan='2' align='center'><input type='submit' id='wpf-search-submit' name='search_submit' value='".__("Start Search", "mingleforum")."'/></td>
						</tr>";
			$o .= "</table></form></div>";
		}
		else{
	...
	function notify_starter($thread, $subject, $content, $date){
		...
		$submitter_email = $submitter->user_email;
			...
			$subject = __("New post in forum:", "mingleforum")." '$subject'.";
	...
	function get_seo_friendly_query()
	{
		...
		$link = str_replace(get_option('siteurl'), '', get_permalink($this->get_pageid()));
	...
	{
		$new = array();
		$link = trim(str_replace(array(get_option('siteurl'), 'index.php/'), '', get_permalink($this->get_pageid())), '/');
	...
	function saving_posts($post_id)
	{
		...
		if($mydata)
		{
			...
			$sql_thread = "INSERT INTO {$this->t_threads} (last_post, subject, parent_id, `date`, status, starter) VALUES('{$date}', '{$post->post_title}', '{$fid}', '{$date}', 'open', '{$user_ID}')";
			$wpdb->query($wpdb->prepare($sql_thread));
			$tid = $wpdb->insert_id;
			$sql_post = "INSERT INTO {$this->t_posts} (text, parent_id, `date`, author_id, subject) VALUES('{$post->post_content}', '{$tid}', '{$date}', '{$user_ID}', '{$post->post_title}')";
