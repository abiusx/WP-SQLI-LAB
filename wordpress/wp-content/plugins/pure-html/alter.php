<?php
	...
	if(!isset($_POST['PureHTMLNOnce'])){
		...
		elseif($action == "add" && $function != ""){
			$sql = "insert into ".$wpdb->prefix."pureHTML_functions (function,name) values('".$function."','".$name."')";
			$results = $wpdb->get_results($wpdb->prepare($sql));
			header("location:".$refer);
		}
		#modify
		elseif($action == "modify" && $function != ""){
			$sql = "update ".$wpdb->prefix."pureHTML_functions set function='".$function."', name='".$name."' where id = ".$id;
			$results = $wpdb->get_results($wpdb->prepare($sql));
			header("location:".$refer);
		}
